...
function poetry2semver(poetry_version, padRelease = true) {
    ...
    if (pre !== null) {
        // trim leading zeros from valid numbers
        pre.number = pre.number.replace((0, regex_1.regEx)(/^0+(\d+)/), '$1');
        parts.push(`-${pre.letter}.${pre.number}`);
    }
    if (post !== null) {
        // trim leading zeros from valid numbers
        post.number = post.number.replace((0, regex_1.regEx)(/^0+(\d+)/), '$1');
        parts.push(`-${post.letter}.${post.number}`);
    }
    if (dev !== null) {
        // trim leading zeros from valid numbers
        dev.number = dev.number.replace((0, regex_1.regEx)(/^0+(\d+)/), '$1');
