...
function extractFromSection(parsedContent, section, cargoRegistries, target, depTypeOverride) {
    const deps = [];
    ...
    Object.keys(sectionContent).forEach((depName) => {
        ...
        if (typeof currentValue !== 'string') {
            ...
            if (version) {
                ...
                nestedVersion = true;
                if (registryName) {
                    const registryUrl = getCargoIndexEnv(registryName) ?? cargoRegistries[registryName];
                    if (registryUrl) {
...
async function extractPackageFile(content, packageFile, _config) {
    ...
    try {
        ...
        logger_1.logger.debug({ err, packageFile }, 'Error parsing Cargo.toml file');
        return null;
    }
    /*
    ...
    */
    ...
    let workspaceDeps = [];
    if (workspaceSection) {
        workspaceDeps = extractFromSection(workspaceSection, 'dependencies', cargoRegistries, undefined, 'workspace.dependencies');
    }
    const deps = [
