...
class GalaxyCollectionDatasource extends datasource_1.Datasource {
    ...
    async getReleases({ packageName, registryUrl, }) {
        ...
        const { val: baseProject, err: baseErr } = await this.http
            ...
            .onError((err) => {
            logger_1.logger.warn({ datasource: this.id, packageName, err }, `Error fetching ${baseUrl}`);
        })
            .unwrap();
        if (baseErr) {
        ...
        const { val: rawReleases, err: versionsErr } = await this.http
            ...
            .onError((err) => {
            logger_1.logger.warn({ datasource: this.id, packageName, err }, `Error fetching ${versionsUrl}`);
        })
            .unwrap();
        if (versionsErr) {
...
tslib_1.__decorate([
    (0, decorator_1.cache)({
        ...
        key: (versionsUrl, basicRelease) => basicRelease.version,
