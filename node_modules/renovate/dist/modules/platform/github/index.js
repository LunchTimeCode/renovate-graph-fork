...
exports.commitFiles = exports.getVulnerabilityAlerts = exports.massageMarkdown = exports.mergePr = exports.reattemptPlatformAutomerge = exports.updatePr = exports.createPr = exports.ensureCommentRemoval = exports.ensureComment = exports.deleteLabel = exports.addReviewers = exports.addAssignees = exports.ensureIssueClosing = exports.ensureIssue = exports.findIssue = exports.getIssue = exports.getIssueList = exports.setBranchStatus = exports.getBranchStatusCheck = exports.getBranchStatus = exports.getBranchPr = exports.findPr = exports.getPrList = exports.getPr = exports.getBranchForceRebase = exports.initRepo = exports.createFork = exports.findFork = exports.listForks = exports.getJsonFile = exports.getRawFile = exports.getRepos = exports.initPlatform = exports.detectGhe = exports.resetConfigs = exports.GitHubMaxPrBodyLen = exports.id = void 0;
...
let config;
let platformConfig;
exports.GitHubMaxPrBodyLen = 60000;
...
async function fetchRepositories() {
    try {
        if (platformConfig.isGHApp) {
            const res = await common_2.githubApi.getJson(`installation/repositories?per_page=100`, {
            ...
            const res = await common_2.githubApi.getJson(`user/repos?per_page=100`, { paginate: 'all' });
            return res.body;
        }
    }
...
async function getBranchProtection(branchName) {
    ...
    const res = await common_2.githubApi.getJson(`repos/${config.repository}/branches/${escapeHash(branchName)}/protection`, { cacheProvider: repository_http_cache_provider_1.repoCacheProvider });
    return res.body;
}
async function getRawFile(fileName, repoName, branchOrTag) {
    ...
    const res = await common_2.githubApi.getJson(url, httpOptions);
...
async function listForks(token, repository) {
    try {
        ...
        const repos = (await common_2.githubApi.getJson(url, {
            token,
            paginate: true,
            pageLimit: 100,
        })).body;
...
async function initRepo({ endpoint, repository, forkCreation, forkOrg, forkToken, renovateUsername, cloneSubmodules, ignorePrAuthor, }) {
...
async function fetchPr(prNo) {
    try {
        const { body: ghRestPr } = await common_2.githubApi.getJson(`repos/${config.parentRepo ?? config.repository}/pulls/${prNo}`);
...
async function findPr({ branchName, prTitle, state = 'all', includeOtherAuthors, }) {
    ...
    if (includeOtherAuthors) {
        ...
        const { body: prList } = await common_2.githubApi.getJson(`repos/${repo}/pulls?head=${repo}:${branchName}&state=open`, { cacheProvider: repository_http_cache_provider_1.repoCacheProvider });
        if (!prList.length) {
...
async function getStatus(branchName, useCache = true) {
    ...
    const { body: status } = await common_2.githubApi.getJson(url, {
        memCache: useCache,
...
async function getBranchStatus(branchName, internalChecksAsSuccess) {
    ...
    try {
        ...
        const checkRunsRaw = (await common_2.githubApi.getJson(checkRunsUrl, opts)).body;
...
async function getStatusCheck(branchName, useCache = true) {
    ...
    return (await common_2.githubApi.getJson(url, { memCache: useCache })).body;
}
...
async function getComments(issueNo) {
    ...
    try {
        const { body: comments } = await common_2.githubApi.getJson(url, {
            paginate: true,
            cacheProvider: repository_http_cache_provider_1.repoCacheProvider,
        });
...
async function createPr({ sourceBranch, targetBranch, prTitle: title, prBody: rawBody, labels, draftPR = false, platformOptions, milestone, }) {
    const body = (0, sanitize_1.sanitize)(rawBody);
...
async function reattemptPlatformAutomerge({ number, platformOptions, }) {
    try {
...
function massageMarkdown(input) {
    if (platformConfig.isGhe) {
        return (0, pr_body_1.smartTruncate)(input, exports.GitHubMaxPrBodyLen);
    }
    const massagedInput = (0, massage_markdown_links_1.massageMarkdownLinks)(input)
        ...
        .replace('> ⚠️ **Warning**\n> \n', '> [!WARNING]\n')
        .replace('> ❗ **Important**\n> \n', '> [!IMPORTANT]\n');
    return (0, pr_body_1.smartTruncate)(massagedInput, exports.GitHubMaxPrBodyLen);
}
...
async function getVulnerabilityAlerts() {
    ...
    try {
        vulnerabilityAlerts = await common_2.githubApi.queryRepoField(query, 'vulnerabilityAlerts', {
