...
const platform = {
    async initPlatform({ endpoint, token, }) {
        ...
        try {
            ...
            if (defaults.version?.includes('gitea-')) {
                defaults.version = defaults.version.substring(defaults.version.indexOf('gitea-') + 6);
    ...
    getPrList() {
        return pr_cache_1.GiteaPrCache.getPrs(gitea_helper_1.giteaHttp, config.repository, botUserName);
    },
    async getPr(number) {
        ...
        if (pr) {
            ...
            if (pr) {
                await pr_cache_1.GiteaPrCache.addPr(gitea_helper_1.giteaHttp, config.repository, botUserName, pr);
            }
        }
    ...
    async findPr({ branchName, prTitle: title, state = 'all', includeOtherAuthors, }) {
    ...
    async createPr({ sourceBranch, targetBranch, prTitle, prBody: rawBody, labels: labelNames, platformOptions, draftPR, }) {
        let title = prTitle;
        ...
        try {
            ...
            if (platformOptions?.usePlatformAutomerge) {
                if (semver_1.default.gte(defaults.version, '1.17.0')) {
                    try {
                        await helper.mergePR(config.repository, gpr.number, {
                            Do: (0, utils_1.getMergeMethod)(platformOptions?.automergeStrategy) ??
                                config.mergeMethod,
                    ...
                    logger_1.logger.debug({ prNumber: gpr.number }, 'Gitea-native automerge: not supported on this version of Gitea. Use 1.17.0 or newer.');
                }
            }
