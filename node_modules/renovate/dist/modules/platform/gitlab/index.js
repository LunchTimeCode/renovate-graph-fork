...
exports.expandGroupMembers = exports.filterUnavailableUsers = exports.ensureCommentRemoval = exports.ensureComment = exports.deleteLabel = exports.addReviewers = exports.addAssignees = exports.ensureIssueClosing = exports.ensureIssue = exports.findIssue = exports.getIssue = exports.getIssueList = exports.setBranchStatus = exports.getBranchStatusCheck = exports.getBranchPr = exports.findPr = exports.massageMarkdown = exports.mergePr = exports.reattemptPlatformAutomerge = exports.updatePr = exports.getPr = exports.createPr = exports.getPrList = exports.getBranchStatus = exports.getBranchForceRebase = exports.initRepo = exports.getJsonFile = exports.getRawFile = exports.getRepos = exports.initPlatform = exports.id = void 0;
...
async function getRepos(config) {
    ...
    try {
        const repos = (await (0, p_map_1.default)(urls, (url) => http_1.gitlabApi.getJson(url, {
            paginate: true,
        }), {
            concurrency: 2,
...
async function initRepo({ repository, cloneSubmodules, ignorePrAuthor, gitUrl, endpoint, includeMirrors, }) {
    config = {};
...
async function getBranchStatus(branchName, internalChecksAsSuccess) {
    ...
    if (!is_1.default.undefined(mrStatus)) {
        branchStatuses.push({
            status: mrStatus,
            name: 'head_pipeline',
        });
...
exports.getPr = getPr;
async function updatePr({ number: iid, prTitle, prBody: description, addLabels, removeLabels, state, platformOptions, targetBranch, }) {
    let title = prTitle;
    ...
    await http_1.gitlabApi.putJson(`projects/${config.repository}/merge_requests/${iid}`, { body });
    if (platformOptions?.autoApprove) {
        await approvePr(iid);
    }
}
exports.updatePr = updatePr;
async function reattemptPlatformAutomerge({ number: iid, platformOptions, }) {
...
async function findPr({ branchName, prTitle, state = 'all', includeOtherAuthors, }) {
    ...
    if (includeOtherAuthors) {
        ...
        const response = await http_1.gitlabApi.getJson(`projects/${config.repository}/merge_requests?source_branch=${branchName}&state=opened`);
...
async function setBranchStatus({ branchName, context, description, state: renovateState, url: targetUrl, }) {
    ...
    if (branchSha) {
        ...
        await http_1.gitlabApi
            .getJsonSafe(commitUrl, schema_1.LastPipelineId)
            .onValue((pipelineId) => {
...
async function getIssue(number, useCache = true) {
    try {
        const issueBody = (await http_1.gitlabApi.getJson(`projects/${config.repository}/issues/${number}`, { memCache: useCache })).body.description;
        return {
            number,
            body: issueBody,
        };
