...
exports.GerritScm = exports.configureScm = void 0;
...
class GerritScm extends default_scm_1.DefaultGitScm {
    ...
    async isBranchConflicted(baseBranch, branch) {
        ...
        if (change) {
            ...
            logger_1.logger.warn(`There is no open change with branch=${branch} and baseBranch=${baseBranch}`);
            return true;
        }
    }
    ...
    async commitAndPush(commit) {
        ...
        const existingChange = await client_1.client
            ...
            .then((res) => res.pop());
        let hasChanges = true;
        const origMsg = typeof commit.message === 'string' ? [commit.message] : commit.message;
        commit.message = [
            ...origMsg,
        ...
        if (commitResult) {
            ...
            if (hasChanges || commit.force) {
                const pushResult = await git.pushCommit({
                    ...
                    targetRef: `refs/for/${commit.baseBranch}%t=sourceBranch-${commit.branchName}`,
                    files: commit.files,
                });
                if (pushResult) {
