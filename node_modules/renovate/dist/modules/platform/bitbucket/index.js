...
exports.mergePr = exports.updatePr = exports.createPr = exports.ensureCommentRemoval = exports.ensureComment = exports.deleteLabel = exports.addReviewers = exports.addAssignees = exports.ensureIssueClosing = exports.getIssueList = exports.ensureIssue = exports.massageMarkdown = exports.findIssue = exports.setBranchStatus = exports.getBranchStatusCheck = exports.getBranchStatus = exports.getBranchPr = exports.getPr = exports.findPr = exports.getPrList = exports.initRepo = exports.getJsonFile = exports.getRawFile = exports.getRepos = exports.initPlatform = exports.id = void 0;
...
async function initPlatform({ endpoint, username, password, token, }) {
    ...
    try {
        const { uuid } = (await bitbucketHttp.getJson('/2.0/user', options)).body;
...
async function initRepo({ repository, cloneSubmodules, ignorePrAuthor, bbUseDevelopmentBranch, }) {
    ...
    try {
        ...
        if (bbUseDevelopmentBranch) {
            ...
            const developmentBranch = (await bitbucketHttp.getJson(`/2.0/repositories/${repository}/branching-model`)).body.development?.branch?.name;
...
async function findPr({ branchName, prTitle, state = 'all', includeOtherAuthors, }) {
    ...
    if (includeOtherAuthors) {
        ...
        const prs = (await bitbucketHttp.getJson(`/2.0/repositories/${config.repository}/pullrequests?q=source.branch.name="${branchName}"&state=open`)).body.values;
        if (prs.length === 0) {
...
async function getStatus(branchName, memCache = true) {
    ...
    return (await bitbucketHttp.getJson(`/2.0/repositories/${config.repository}/commit/${sha}/statuses`, {
        paginate: true,
        memCache,
    })).body.values;
}
        ...
        for (const msg of err.body.error.fields.reviewers) {
            ...
            if (msg === MSG_MALFORMED_REVIEWERS_LIST) {
                ...
                for (const reviewer of reviewers) {
                    const reviewerUser = (await bitbucketHttp.getJson(`/2.0/users/${reviewer.uuid}`, { memCache: true })).body;
...
async function isAccountMemberOfWorkspace(reviewer, repository) {
    ...
    try {
        await bitbucketHttp.get(`/2.0/workspaces/${workspace}/members/${reviewer.uuid}`, { memCache: true });
        return true;
    }
    catch (err) {
...
async function createPr({ sourceBranch, targetBranch, prTitle: title, prBody: description, platformOptions, }) {
    ...
    if (platformOptions?.bbUseDefaultReviewers) {
        const reviewersResponse = (await bitbucketHttp.getJson(`/2.0/repositories/${config.repository}/effective-default-reviewers`, {
            paginate: true,
        })).body;
    ...
    try {
        ...
        await pr_cache_1.BitbucketPrCache.addPr(bitbucketHttp, config.repository, renovateUserUuid, pr);
        return pr;
    }
    catch (err) /* istanbul ignore next */ {
...
async function updatePr({ number: prNo, prTitle: title, prBody: description, state, targetBranch, }) {
    ...
    try {
        ...
        await bitbucketHttp.putJson(`/2.0/repositories/${config.repository}/pullrequests/${prNo}`, { body });
    }
    catch (err) {
        // Try sanitizing reviewers
