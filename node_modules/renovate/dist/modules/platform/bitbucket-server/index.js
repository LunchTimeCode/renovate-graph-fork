...
async function initPlatform({ endpoint, token, username, password, }) {
    if (!endpoint) {
    ...
    if (!(username && password) && !token) {
        ...
        throw new Error('Init: You must either configure a Bitbucket Server password or a HTTP access token');
    }
    ...
    try {
        ...
        if (process.env.RENOVATE_X_PLATFORM_VERSION) {
            ...
            const { version } = (await bitbucketServerHttp.getJson(`./rest/api/1.0/application-properties`)).body;
...
async function getRepos() {
    ...
    try {
        ...
        const result = repos.map((r) => `${r.project.key}/${r.slug}`);
...
async function getPr(prNo, refreshCache) {
    ...
    const res = await bitbucketServerHttp.getJson(`./rest/api/1.0/projects/${config.projectKey}/repos/${config.repositorySlug}/pull-requests/${prNo}`, { memCache: !refreshCache });
    const pr = {
...
async function findPr({ branchName, prTitle, state = 'all', refreshCache, includeOtherAuthors, }) {
    ...
    if (includeOtherAuthors) {
        ...
        const query = (0, url_1.getQueryString)(searchParams);
        const prs = await utils.accumulateValues(`./rest/api/1.0/projects/${config.projectKey}/repos/${config.repositorySlug}/pull-requests?${query}`, 'get', {}, 1);
        if (!prs.length) {
...
function getStatusCheck(branchName, memCache = true) {
    ...
    // TODO: types (#22198)
    `./rest/build-status/1.0/commits/${branchCommit}`, 'get', { memCache });
}
// https://docs.atlassian.com/bitbucket-server/rest/6.0.0/bitbucket-build-rest.html#idp2
