...
async function cleanup() {
    ...
    try {
        ...
        logger_1.logger.debug(`Verifying and cleaning cache: ${cacheFileName}`);
        await cacache_1.default.verify(cacheFileName);
        const durationMs = Math.round(Date.now() - startTime);
        logger_1.logger.debug(`Deleted ${deletedCount} of ${totalCount} file cached entries in ${durationMs}ms`);
    }
    catch (err) /* istanbul ignore next */ {
        logger_1.logger.warn({ err }, 'Error cleaning up expired file cache');
    }
}
