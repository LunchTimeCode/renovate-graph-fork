...
class PackagePatternsMatcher extends base_1.Matcher {
    matches({ depName, packageName }, packageRule) {
        ...
        if (matchPatternsAgainstName(matchPackagePatterns, depName)) {
            logger_1.logger.once.warn({ packageRule, packageName, depName }, 'Use matchDepPatterns instead of matchPackagePatterns');
            return true;
        }
        return false;
    }
    excludes({ depName, packageName }, packageRule) {
        ...
        if (matchPatternsAgainstName(excludePackagePatterns, depName)) {
            logger_1.logger.once.warn({ packageRule, packageName, depName }, 'Use excludeDepPatterns instead of excludePackagePatterns');
            return true;
        }
        return false;
    }
