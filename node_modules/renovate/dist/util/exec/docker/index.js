...
exports.generateDockerCommand = exports.removeDanglingContainers = exports.removeDockerContainer = exports.getDockerTag = exports.resetPrefetchedImages = exports.prefetchDockerImage = exports.sideCarImage = void 0;
...
async function getDockerTag(packageName, constraint, scheme) {
    ...
    logger_1.logger.debug({ packageName, scheme, constraint }, `Found version constraint - checking for a compatible image to use`);
    ...
    if (imageReleases?.releases) {
        ...
        if (version) {
            logger_1.logger.debug({ packageName, scheme, constraint, version }, `Found compatible image version`);
            return version;
        }
    }
    else {
    ...
    logger_1.logger.warn({ packageName, constraint, scheme }, 'Failed to find a tag satisfying constraint, using "latest" tag instead');
    return 'latest';
}
