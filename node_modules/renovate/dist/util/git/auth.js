...
function getGitAuthenticatedEnvironmentVariables(originalGitUrl, { token, username, password, hostType, matchHost }, environmentVariables) {
    ...
    if (token) {
        authenticationRules = getAuthenticationRulesWithToken(originalGitUrl, hostType, token);
    }
    else {
        const encodedUsername = encodeURIComponent(username);
        const encodedPassword = encodeURIComponent(password);
        authenticationRules = getAuthenticationRules(originalGitUrl, `${encodedUsername}:${encodedPassword}`);
    }
...
function getGitEnvironmentVariables(additionalHostTypes = []) {
    ...
    const gitHubHostRule = (0, host_rules_1.find)({
        hostType: 'github',
        url: 'https://api.github.com/',
    });
    if (gitHubHostRule?.token) {
        environmentVariables = getGitAuthenticatedEnvironmentVariables('https://github.com/', gitHubHostRule);
    }
...
function addAuthFromHostRule(hostRule, env) {
    ...
    if ((0, url_1.isHttpUrl)(httpUrl)) {
        logger_1.logger.trace(`Adding Git authentication for ${httpUrl} using token auth.`);
