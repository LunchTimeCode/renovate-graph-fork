...
function checkEnv() {
    ...
    if (process.release?.name !== 'node' || !process.versions?.node) {
        logger_1.logger[process.env.RENOVATE_X_IGNORE_NODE_WARN ? 'info' : 'warn']({ release: process.release, versions: process.versions }, 'Unknown node environment detected.');
    }
        ...
        logger_1.logger[process.env.RENOVATE_X_IGNORE_NODE_WARN ? 'info' : 'warn']({ versions: process.versions }, `Please upgrade the version of Node.js used to run Renovate to satisfy "${rangeNext}". Support for your current version will be removed in Renovate's next major release.`);
    }
}
...
async function start() {
    ...
    try {
        ...
        await (0, instrumentation_1.instrument)('config', async () => {
            ...
            if (config?.globalExtends) {
                ...
                if (process.env.RENOVATE_X_EAGER_GLOBAL_EXTENDS) {
                    config = (0, config_1.mergeChildConfig)(await resolveGlobalExtends(config.globalExtends), config);
                }
                else {
        ...
        if (logLevel === 'info') {
            logger_1.logger.info(`Renovate was run at log level "${logLevel}". Set LOG_LEVEL=debug in environment variables to see extended debug logs.`);
        }
    }
