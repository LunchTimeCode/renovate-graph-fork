...
function warnOnUnsupportedOptions(config) {
    ...
    if (config.expandCodeOwnersGroups && !platform_1.platform.expandGroupMembers) {
        // TODO: types (#22198)
        const platform = global_1.GlobalConfig.get('platform');
        logger_1.logger.warn(`Configuration option 'expandCodeOwnersGroups' is not supported on the current platform '${platform}'.`);
    }
}
async function initRepo(config_) {
    ...
    await (0, git_1.cloneSubmodules)(!!config.cloneSubmodules);
    return config;
}
