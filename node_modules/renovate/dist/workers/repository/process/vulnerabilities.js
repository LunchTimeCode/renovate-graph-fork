...
class Vulnerabilities {
    ...
    async initialize() {
        ...
        const token = (0, check_token_1.findGithubToken)((0, host_rules_1.find)({
            hostType: 'github',
            url: 'https://api.github.com/',
        }));
        this.osvOffline = await osv_offline_1.OsvOffline.create(token);
    }
    static async create() {
    ...
    async fetchDependencyVulnerability(packageFileConfig, dep) {
        ...
        try {
            ...
            logger_1.logger.warn({ err }, `Error fetching vulnerability information for ${packageName}`);
            return null;
        }
    }
    ...
    vulnerabilityToPackageRules(vul) {
        ...
        if (is_1.default.nullOrUndefined(fixedVersion)) {
            logger_1.logger.info(`No fixed version available for vulnerability ${vulnerability.id} in ${packageName} ${depVersion}`);
            return null;
        }
