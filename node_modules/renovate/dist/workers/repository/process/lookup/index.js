...
function getTimestamp(versions, version, versioning) {
    return versions.find((v) => versioning.isValid(v.version) && versioning.equals(v.version, version))?.releaseTimestamp;
}
async function lookupUpdates(inconfig) {
    ...
    try {
        ...
        const isValid = is_1.default.string(compareValue) && versioning.isValid(compareValue);
        if (unconstrainedValue || isValid) {
            ...
            // TODO #22198
            currentVersion ??=
                (0, current_1.getCurrentVersion)(compareValue, config.lockedVersion, versioning, rangeStrategy, latestVersion, nonDeprecatedVersions) ??
                    (0, current_1.getCurrentVersion)(compareValue, config.lockedVersion, versioning, rangeStrategy, latestVersion, allVersions.map((v) => v.version));
            if (!currentVersion) {
            ...
            const currentVersionTimestamp = getTimestamp(allVersions, currentVersion, versioning);
            if (is_1.default.nonEmptyString(currentVersionTimestamp)) {
                res.currentVersionTimestamp = currentVersionTimestamp;
                if (config.packageRules?.some((rules) => is_1.default.nonEmptyString(rules.matchCurrentAge))) {
                    // Reapply package rules to check matches for matchCurrentAge
                    config = (0, package_rules_1.applyPackageRules)({ ...config, currentVersionTimestamp });
                }
            }
            if (compareValue &&
                currentVersion &&
            ...
            for (const [bucket, releases] of Object.entries(buckets)) {
                ...
                const update = await (0, generate_1.generateUpdate)(config, compareValue, versioning, 
                // TODO #22198
