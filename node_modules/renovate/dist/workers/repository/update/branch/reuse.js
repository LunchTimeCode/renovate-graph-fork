...
async function shouldKeepUpdated(config, baseBranch, branchName) {
    const keepUpdatedLabel = config.keepUpdatedLabel;
    if (!keepUpdatedLabel) {
        return false;
    }
    const branchPr = await platform_1.platform.getBranchPr(config.branchName, config.baseBranch);
    ...
    if (config.rebaseWhen === 'behind-base-branch' ||
        ...
        (config.rebaseWhen === 'auto' &&
            (config.automerge === true ||
                (await platform_1.platform.getBranchForceRebase?.(config.baseBranch))))) {
